...
contract EscrowByAgent is Ownable, ReentrancyGuard, IEscrowByAgent {
    ...
    function depositByETH(address _recipient, address _agent) external override payable returns (uint256) {
        require(msg.value > 0, "amount invalid");
        require(_recipient != address(0x0) && _agent != address(0x0), "address invalid");
        return _deposit(address(0x0), msg.sender, _recipient, _agent, msg.value);
    }

    function deposit(IERC20 _token, address _recipient, address _agent, uint256 _amount) external override returns (uint256) {
    ...
    function release(uint256 _poolId) external onlyAgent(_poolId) override nonReentrant returns (bool) {
        require(_poolId < poolCount, "poolId invalid");
        Pool memory pool = pools[_poolId];
        require(pool.amount > 0, "no money in pool");
        require(!pool.isReleased, "already release");
    ...
    function cancel(uint256 _poolId) external override nonReentrant returns (bool) {
        require(_poolId < poolCount, "poolId invalid");
        ...
        if (!refundStatus.recipient) {
            require(block.timestamp > pool.createdAt + cancelLockTime, "during cancelLock");
        }

        require(msg.sender == pool.sender || refundStatus.sender, "sender didn't approve");
